import { getDb, voices, voiceSamples } from "@illvenn/db"
...
export async function sampleRoutes(app: FastifyInstance) {
	...
	app.get("/:sampleId", async (request: FastifyRequest, reply: FastifyReply) => {
	...
	app.delete("/:sampleId", async (request: FastifyRequest, reply: FastifyReply) => {
		const { voiceId, sampleId } = request.params as { voiceId: string; sampleId: string }
